...
| `fixture-citations.md` | One citation per verdict of `skills/validate/check-citations.py` that a checked-in document can legitimately contain — verified, a bare `:NN` continuation, anchor-moved, anchor-absent, past-EOF, no-such-file, ambiguous bare filename, a filename that is a suffix of another, non-unique anchor, line-zero, unanchored, and an anchor that wrapped onto the next line (which is `unverifiable`, not bound). Verdicts needing a controlled tree or a malformed document — reversed ranges, orphan continuations, fenced citations, ignored shadow files, submodules, a tracked-but-deleted file — are built as throwaway inputs inside `tests/validate/test-citations.sh` instead. Unlike the other fixtures this one is not run through `/validate` by hand: that test asserts the verdict for each citation, keyed on the citation's own text, so the expected results are executable rather than described. |
